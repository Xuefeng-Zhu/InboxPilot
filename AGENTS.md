...
insforge/migrations/          23 SQL migration files + seed.sql
...
- **Migration count: 23 files.** This includes numbered migrations `001` through `021` plus two timestamped job-trigger migrations; preserve the documented application order because the second timestamped file drops the first file's unreliable trigger.
