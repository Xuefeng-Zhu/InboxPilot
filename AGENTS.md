...
insforge/migrations/          22 SQL migration files + seed.sql
...
- **Migration count: 22 files.** This includes numbered migrations `001` through `020` plus two timestamped job-trigger migrations; preserve the documented application order because the second timestamped file drops the first file's unreliable trigger.
