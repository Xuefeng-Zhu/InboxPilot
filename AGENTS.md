...
insforge/migrations/          20 SQL migration files + seed.sql
...
- **Migration count: 20 files.** This includes numbered migrations `001` through `018` plus two timestamped job-trigger migrations; preserve the documented application order because the second timestamped file drops the first file's unreliable trigger.
