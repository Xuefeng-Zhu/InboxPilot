...
insforge/migrations/          21 SQL migration files + seed.sql
...
- **Migration count: 21 files.** This includes numbered migrations `001` through `019` plus two timestamped job-trigger migrations; preserve the documented application order because the second timestamped file drops the first file's unreliable trigger.
