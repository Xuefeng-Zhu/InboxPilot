...
| `insforge/migrations/022_atomic_ai_decision_finalization.sql` | Commits each AI decision and terminal conversation state together, with an exact pending-draft pointer |
| `insforge/migrations/023_secure_realtime_channels.sql` | Restricts organization subscriptions to members, validates visitor widget channels, and blocks browser publishing |

Apply all 25 files via the InsForge SQL editor or migrations API in the order shown above. Migration `014` intentionally does not change bucket visibility: after applying it, mark the existing `knowledge-files` bucket **private** in the InsForge dashboard. Keep knowledge object keys under `<organization-id>/documents/...`; the migration's storage policies depend on that prefix. Pause scheduled `process-jobs` invocations and let any active invocation finish before applying `018`; deploy the source-bound routes/functions before resuming the schedule. Migration `019` removes direct browser mutation access to server-produced AI decisions. Apply `020` before deploying the owner-bound approval/regeneration routes that call its RPCs, and apply `021` before deploying status handlers that use atomic delivery advancement. For an existing deployment, pause every scheduled or manual worker trigger again and wait until no `process_ai_message` job is `claimed`; migration `022` aborts otherwise. Apply `022`, deploy the atomic-finalization worker, and only then resume processing. Apply `023` before exposing organization realtime subscriptions to authenticated clients.
