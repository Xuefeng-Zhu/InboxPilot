  ...
  try {
    ...
    if (conversation.status !== 'open' && conversation.status !== 'escalated') {
      return NextResponse.json(
        { error: 'Only open or escalated conversations can be resolved' },
        { status: 409 },
      );
    }
    ...
    try {
      ...
      if (auditResult.error) {
        auditWarning = `Conversation was resolved, but its audit log failed: ${auditResult.error.message}`;
        console.error('resolve-conversation: failed to write audit log', auditResult.error.message);
      }
    } catch (error) {
      const detail = error instanceof Error ? error.message : String(error);
      auditWarning = `Conversation was resolved, but its audit log failed: ${detail}`;
      console.error('resolve-conversation: failed to write audit log', detail);
    }
