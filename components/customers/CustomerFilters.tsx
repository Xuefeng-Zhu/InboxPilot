  ...
  return (
    <div className="mt-6 flex items-center gap-3 flex-wrap">
      <div className="relative flex-1 min-w-[200px] max-w-sm">
        <svg className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
      ...
      <div className="flex items-center gap-1.5">
        {(['all', 'email', 'phone'] as const).map((filter) => (
          <button
            key={filter}
          ...
          >
            {filter === 'all' ? 'All Users' : filter === 'email' ? 'Email' : 'Phone'}
          </button>
        ))}
      </div>
