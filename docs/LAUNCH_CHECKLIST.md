...
| 8.2 | All SQL migrations are reversible: either a paired `-- @down` block, or a documented `DROP TABLE … CASCADE` chain | ☐ | `grep -L "@down" insforge/migrations/*.sql` returns no results. Down blocks are tested in staging (see 8.4). **Status as of 2026-06-07:** all 3 existing migrations (001, 002, 003) have `-- @down` blocks (added by `t_ops_runbook`). **Adoption note (2026-06-07, t_99f64457):** the **only** supported path for applying a migration is `scripts/apply-migrations.sh` (not raw `psql -f`, which would run the `@down` block in the up direction and leave the schema in a half-state). Rollback is `scripts/apply-migrations.down.sh --last N`. Both ship with 25 unit tests in `__tests__/apply-migrations.test.ts` (offline) and a CI gate in `.github/workflows/migrate-integration.yml` (live apply → seed → vitest → rollback → re-apply). | DEVOPS + ENG-LEAD |
