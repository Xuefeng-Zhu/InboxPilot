...
- [ ] All 25 migration files applied in the documented order (through `023`, including both timestamped job-trigger migrations).
...
- [ ] For an existing deployment, every scheduled or manual worker trigger was paused and no `process_ai_message` job remained `claimed` before `022` (the migration aborts otherwise); the atomic-finalization worker was deployed before processing resumed.
- [ ] Migration `023` applied so organization realtime subscriptions are tenant-isolated and browser roles cannot publish forged events.
