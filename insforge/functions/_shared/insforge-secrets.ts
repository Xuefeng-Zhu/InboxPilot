 ...
 * Deno function (process-jobs, etc.) needs to call a provider.
 *
 ...
 *
 * Used by:
 *   - insforge/functions/process-jobs/index.ts   (send_outbound_message)
 *
 * Deno convention: take `baseUrl` and `serviceRoleKey` as function parameters
 * (not from `Deno.env.get()` inside the function body) so the module is
 * testable and the caller controls how the secrets are injected. Mirrors
 * the surrounding `_shared/create-db-client.ts` and `_shared/create-realtime-publisher.ts`
 * pattern.
 */
