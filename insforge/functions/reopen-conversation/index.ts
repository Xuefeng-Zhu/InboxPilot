/**
 * reopen-conversation — Reopens a closed/resolved conversation.
 *
 ...
 * 4. Set conversation status back to "open" and ai_state to "needs_human"
 * 5. Record audit log entry for "conversation_reopened"
...
export default async function (req: Request): Promise<Response> {
  const ctx = newRequestContext('reopen-conversation', req);
  try {
    const response = await withRequest(ctx, async () => {
      ...
      log({ ...ctx, level: 'info', msg: 'conversation reopened', status_after: conversation.status });
