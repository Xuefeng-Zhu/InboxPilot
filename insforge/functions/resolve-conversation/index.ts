/**
 * resolve-conversation — Resolves a conversation (mark as resolved).
 *
...
export default async function (req: Request): Promise<Response> {
  const ctx = newRequestContext('resolve-conversation', req);
  try {
    const response = await withRequest(ctx, async () => {
      ...
      log({ ...ctx, level: 'info', msg: 'conversation resolved', status_after: conversation.status });
