 ...
 * signing secret is resolved SERVER-SIDE from the receiving phone
 * number (sms_phone_numbers → sms_provider_accounts.credentials_secret_id
 ...
 * 2. Determine SMS provider from x-provider header; refuse mock in production
 * 3. Build the provider registry and pick the adapter for x-provider
 * 4. Parse the inbound payload to learn the receiving phone number
 * 5. Construct the InsForge DB + secrets clients (service role)
 * 6. Resolve the per-org signing secret by receiving phone number
 *    (refuses the request if the number is unknown, the provider
 ...
 * 9. Delegate to processInboundSms()
...
export default async function (req: Request): Promise<Response> {
  try {
    ...
    //    so we know the `to` phone number. The body and the parsed `to`
    //    are server-derived; the only caller-derived signal is the HTTP
    ...
    //    the receiving phone number — never from a request header.
