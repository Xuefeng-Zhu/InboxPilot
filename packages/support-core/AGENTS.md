...
__tests__/
  ...
  integration/  – 1 opt-in live seed suite plus 4 integration stubs (require real DB); live RLS coverage is in root `__tests__/insforge/`
```
...
- **Integration**: Require a disposable linked `qa-*` InsForge branch. Seed and RLS suites run with `npm run test:integration:seed` and `npm run test:integration:rls`; normal test runs skip remote mutations. Four support-core files remain scenario stubs.
