...
| `AiAgentService` | AI pipeline: load settings → embed → `matchChunks` → **escalation engine first** → if escalated, persist + audit; else LLM → Zod-parse → handle `draft_only`/`auto_reply`/low-confidence. `AiDecisionRecorder` persists one decision per source job, queues chunk refs, and records audit metadata; the worker performs inline auto-send with a queued fallback. | `ConversationRepo, MessageRepo, KnowledgeRepo, AiSettingsRepo, AiDecisionRepo, EscalationEngine, AiClient, JobQueue, AuditLogRepo` |
| `KnowledgeIngestionService` | RAG: snapshot revision → `processing` → optional `FileContentFetcher` → chunk/embed → atomically replace only if current → `ready` (or preserve last-good chunks on failure) → audit | `KnowledgeRepo, AiClient, AuditLogRepo, FileContentFetcher?, AiSettingsRepo?` |
...
| `PostgresJobQueue` | Implements `JobQueue` against `support_jobs`: lifetime/active idempotency policies, atomic `claim`, exponential backoff, conditional completion/quarantine, claim leases, and dead-lettering | `DatabaseClient` |
...
- **Error handling: throw clear errors on repository failures**; delivery and worker boundaries use typed errors only where retryability must be explicit (`ProviderSendOutcomeUnknownError`, `OutboundMessagePostDispatchError`, `NonRetryableJobError`).
